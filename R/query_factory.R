...
get_key <- function(method) {
  ...
  if (key == "") {
    stop(paste0("An API Key is needed to use the '", method, "' method.
...
create_api_parameter <- function(method_name, param_name, value) {
  ...
  if ((length(api_parameter_name) == 0)) {
    return(list())
  }
  if (is.na(api_parameter_name)) {
    return(list())
  }
...
get_api_query <- function(method, generic_parameters = list(), custom_parameters = list()) {
  ...
  for (generic_parameter_name in names(generic_parameters)) {
    main_api_parameters <- c(
      main_api_parameters,
      create_api_parameter(
        method, generic_parameter_name,
